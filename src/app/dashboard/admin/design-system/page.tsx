...
const Section = ({ title, children, flush = false }: { title: string; children: React.ReactNode; flush?: boolean }) => (
  <section className="space-y-3">
    <h2 className="font-serif text-xl font-semibold text-gray-800">{title}</h2>
    <div className={`rounded-xl border bg-white ${flush ? 'px-2 py-4' : 'p-6'}`}>{children}</div>
  </section>
);

type Swatch = { label: string; color: string; light?: boolean };

const PaletteRow = ({ name, swatches }: { name: string; swatches: Swatch[] }) => (
  <div className="flex items-center gap-2">
    ...
    <div className="flex gap-1.5">
      {swatches.map(({ label, color, light }) => (
        ...
        >
          <span className={`text-[9px] font-medium leading-tight ${light ? 'text-gray-600' : 'text-white'}`}>{label}</span>
        </div>
      ))}
    </div>
  </div>
    ...
    <div className="mx-auto max-w-4xl space-y-10 px-6 py-10">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Page privée — palette, tokens et composants UI avant intégration.
        </p>
      </div>
      ...
      <Section title="Design System" flush>
        <div className="space-y-6 px-2">
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Propositions de palette</p>
            <div className="space-y-2">
              {palettes.map((p) => (
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Typographie</p>
            <div className="space-y-0.5">
              <p className="font-serif text-2xl font-bold text-foreground">Lora — titres</p>
              <p className="font-sans text-base text-foreground">Inter — corps de texte</p>
              <p className="font-sans text-sm text-muted-foreground">Inter muted — texte secondaire</p>
            </div>
          </div>
      ...
      <Section title="Badge">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <p className="text-xs font-medium uppercase tracking-wide text-gray-400">Shadcn defaults</p>
          ...
          <div className="space-y-3">
            ...
            {[
              { label: 'A — Sage design system', dej: 'bg-accent text-accent-foreground' },
              ...
              <div key={label} className="flex items-center gap-3">
                ...
                <div className="flex gap-2">
                  <span className="inline-flex items-center rounded-3xl px-2 py-0.5 text-xs font-medium bg-primary text-primary-foreground">Dîner</span>
                  <span className={`inline-flex items-center rounded-3xl px-2 py-0.5 text-xs font-medium ${dej}`}>Déjeuner</span>
                  <span className="inline-flex items-center rounded-3xl px-2 py-0.5 text-xs font-medium bg-amber-100 text-amber-700">Dessert</span>
                  <span className="inline-flex items-center rounded-3xl px-2 py-0.5 text-xs font-medium bg-orange-100 text-orange-700">Petit-déj.</span>
                </div>
              </div>
            ))}
          </div>
